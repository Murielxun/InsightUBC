...
export default class Server {
    ...
    private static getDatasets(req: restify.Request, res: restify.Response, next: restify.Next) {
        Server.insightFacade.listDatasets().then((result: InsightDataset[]) => {
    ...
    private static put(req: restify.Request, res: restify.Response, next: restify.Next) {
        try {
            ...
            Server.insightFacade.addDataset(req.params.id, content, req.params.kind).then((result: string[]) => {
                res.json(200, {
                    result: result
                });
                return next();
    ...
    private static delete(req: restify.Request, res: restify.Response, next: restify.Next) {
        Server.insightFacade.removeDataset(req.params.id).then((result: string) => {
            res.json(200, {result: result});
            return next();
        }).catch((err: Error) => {
